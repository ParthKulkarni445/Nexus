...
function formatMoney(value: number, mode: SeasonType) {
  ...
  return mode === "intern" ? `${value.toFixed(2)}` : `${value.toFixed(2)}`;
}
...
function getInsight(current: SeasonCard, previous?: SeasonCard | null) {
  ...
  return {
    text: `${delta > 0 ? "+" : ""}${delta} students placed vs previous`,
    up: delta >= 0,
  };
}
    ...
    <div className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 h-56">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart
            data={data}
            margin={{ top: 12, right: 28, left: 20, bottom: 6 }}
          >
            <CartesianGrid
              strokeDasharray="3 3"
              stroke="#E2E8F0"
    ...
    <div className="h-full overflow-y-auto px-4 pt-6 hide-scrollbar xl:h-full">
      <div className="space-y-6 pb-0">
        {error ? (
          <EmptyState
            icon={Building2}
            title="Unable to load drives page"
            description={error}
          />
        ) : null}
        ...
        <section className="space-y-6">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`rounded-2xl p-2 ${active ? "bg-[#EFF6FF] text-[#1D4ED8]" : "bg-slate-100 text-slate-600"}`}
                        >
                          <Icon size={18} />
                        </div>
                        <div>
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm space-y-4">
            ...
            {seasonsOfType.length === 0 ? (
              <EmptyState
                icon={Users}
                title={`No ${seasonType} seasons found`}
                description={`No ${seasonType} seasons are available yet.`}
              />
            ) : (
                      ...
                      <div className="mt-5 grid gap-3 sm:grid-cols-3">
                        <div className="rounded-2xl bg-slate-50 p-3">
                          ...
                          <p className="mt-1 text-lg font-semibold text-slate-950">
                            {season.companiesInSeason}
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-3">
                          <p className="text-xs text-slate-500">Placed</p>
                          <p className="mt-1 text-lg font-semibold text-slate-950">
                            {season.studentsPlaced}
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-3">
                          ...
                          <p className="mt-1 text-base font-semibold text-slate-950">
                            {formatMoney(season.avgCompensation, seasonType)}
                          </p>
                        </div>
                      </div>

                      <div
                        className={`mt-4 flex items-center gap-2 text-sm ${insight.up ? "text-emerald-700" : "text-amber-700"}`}
                      >
                        {insight.up ? (
                          <TrendingUp size={16} />
                        ) : (
