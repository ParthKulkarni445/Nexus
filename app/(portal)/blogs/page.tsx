...
import BlogsLoadingView from "./BlogsLoadingView";
...
function mapApiBlog(blog: ApiBlog): BlogPost {
  const source: BlogSource = blog.author.role === "student" ? "student" : "tpo";
  ...
  return {
    ...
    currentUserVote: blog.currentUserVote ?? null,
  };
}
...
export default function BlogsPage() {
  ...
  const [canViewModeration, setCanViewModeration] = useState(true);
  ...
  useEffect(() => {
    ...
    async function loadBlogs() {
      ...
      try {
        const [publishedResponse, moderationResponse] = await Promise.all([
          fetch("/api/v1/blogs?limit=100", {
            cache: "no-store",
            signal: controller.signal,
          }),
          fetch("/api/v1/admin/blogs/moderation?status=pending", {
            cache: "no-store",
            signal: controller.signal,
          }),
        ]);
        ...
        const publishedPayload =
          (await publishedResponse.json()) as ApiEnvelope<ApiBlog[]>;
        const publishedBlogs = (publishedPayload.data ?? []).map(mapApiBlog);
        setBlogs(publishedBlogs);

        if (moderationResponse.ok) {
          const moderationPayload =
            (await moderationResponse.json()) as ApiEnvelope<ApiBlog[]>;
          const queueItems = (moderationPayload.data ?? [])
            .map(mapApiBlog)
            .filter(
              (blog) =>
                blog.source === "student" && blog.moderation !== "approved",
            );
          setModerationQueue(queueItems);
          setCanViewModeration(true);
        } else if (
          moderationResponse.status === 401 ||
          moderationResponse.status === 403
        ) {
          setModerationQueue([]);
          setCanViewModeration(false);
        } else {
          setModerationQueue([]);
          setCanViewModeration(false);
        }
        ...
        setLoadError("Unable to load blogs right now. Please try again.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadCompanies() {
      try {
        ...
        const payload = (await response.json()) as ApiEnvelope<ApiCompany[]>;
        ...
        console.error("Error loading companies for blog creation:", error);
      }
    }
